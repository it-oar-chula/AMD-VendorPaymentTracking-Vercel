...
- ✅ ค้นหาด้วยเลขประจำตัวผู้เสียภาษีเท่านั้น
- ✅ ตรวจรูปแบบเลขผู้เสียภาษี: ต้องเป็นตัวเลข 13 หลัก และระบบลบช่องว่างให้อัตโนมัติ
- ✅ จับคู่ไฟล์ `tax-id*` กับ `Payment_Detail_Report*` ผ่านเลขเอกสาร
- ✅ นำผลลัพธ์จากแถวใน `Payment_Detail_Report` มาแสดงเหมือนเดิม
...
```
GET /api/search?q=3100200097940
```

**Purpose:** ใช้จาก Frontend Web Interface

**พฤติกรรมจริงของโค้ด:**
- ค้นหาได้ด้วยเลขประจำตัวผู้เสียภาษีเท่านั้น
- เลขผู้เสียภาษีต้องเป็นตัวเลข 13 หลัก ระบบจะลบช่องว่างออกให้อัตโนมัติ
- อ่านไฟล์ที่ขึ้นต้นด้วย `tax-id` แล้วดูคอลัมน์ `เลขที่ภาษี 3`
- นำคอลัมน์ `เลขเอกสาร` จากไฟล์ `tax-id*` ไปจับคู่กับคอลัมน์ `เลขที่อ้างอิงผู้รับเงิน` ใน `Payment_Detail_Report*`
- ค่าใน `เลขที่อ้างอิงผู้รับเงิน` เช่น `2026/2350000923` จะถูกตัดให้เหลือ `2350000923` ก่อนจับคู่
  ...
  "results": [
    {
      ...
      "เลขประจำตัวผู้เสียภาษี": "3100200097940",
...
```
GET https://vendor-tracking.vercel.app/api/n8n/search?q=3100200097940
Authorization: Bearer <API_KEY>
```
...
ระบบอ่านไฟล์ใน SharePoint folder เดียวกันที่ขึ้นต้นด้วย:
